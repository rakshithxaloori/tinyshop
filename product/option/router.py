from typing import Annotated
from fastapi import APIRouter, Depends, Form


from product.option import schema, crud
from utils.dependencies import ShopIDDep, LivemodeDep, is_livemode


router = APIRouter(prefix="/v1/options")


def create_option_form(
    name: Annotated[str, Form()],
    values: Annotated[list[str], Form()],
):
    return schema.OptionCreate(name=name, values=values)


def update_option_form(
    name: Annotated[str | None, Form()],
    values: Annotated[list[str] | None, Form()],
):
    return schema.OptionUpdate(name=name, values=values)


@router.post("/{product_id}/options", response_model=schema.Option)
def create_option(
    x_shop_id: ShopIDDep,
    x_livemode: LivemodeDep,
    product_id: str,
    option: Annotated[schema.OptionCreate, Depends(create_option_form)],
):
    print("CREATE", option)
    new_option = crud.create_option(
        x_shop_id,
        x_livemode,
        product_id,
        option,
    )
    return new_option


@router.post("/{product_id}/options/{option_id}", response_model=schema.Option)
def update_option(
    x_shop_id: ShopIDDep,
    x_livemode: LivemodeDep,
    product_id: str,
    option_id: str,
    option: Annotated[schema.OptionUpdate, Depends(update_option_form)],
):
    updated_option = crud.update_option(
        x_shop_id,
        x_livemode,
        product_id,
        option_id,
        option,
    )
    return updated_option


@router.get("/{product_id}/options/{option_id}", response_model=schema.Option)
def retrieve_option(
    x_shop_id: ShopIDDep,
    x_livemode: LivemodeDep,
    product_id: str,
    option_id: str,
):
    option = crud.retrieve_option(
        x_shop_id,
        x_livemode,
        product_id,
        option_id,
    )
    return option


@router.get("/{product_id}/options", response_model=schema.OptionList)
def list_options(
    x_shop_id: ShopIDDep,
    x_livemode: LivemodeDep,
    product_id: str,
):
    # TODO skip, limit
    all_options = crud.list_options(
        x_shop_id,
        x_livemode,
        product_id,
    )
    return schema.OptionList(
        url="/v1/options/{product_id}/options".format(product_id=product_id),
        has_more=False,  # TODO
        data=all_options,
    )


@router.delete("/{product_id}/options/{option_id}", response_model=schema.OptionDelete)
def delete_option(
    x_shop_id: ShopIDDep,
    x_livemode: LivemodeDep,
    product_id: str,
    option_id: str,
):
    deleted_id = crud.delete_option(
        x_shop_id,
        x_livemode,
        product_id,
        option_id,
    )
    deleted_option = schema.OptionDelete(
        id=option_id,
        deleted=deleted_id is not None,
    )
    return deleted_option
